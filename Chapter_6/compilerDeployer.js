...
let input = fs.readFileSync(dir + '/ConstructorContract.sol');
...
const bytecode = output.contracts[':ConstructorContract'].bytecode;
const abi = output.contracts[':ConstructorContract'].interface;

console.log('saving ABI');

try {
  fs.writeFileSync(dir + "/ConstructorContract.json", abi)
} catch (err) {
